...
InitGame.prototype.awake = function() {
    //qici.config.remoteLogUrl = 'http://localhost:8900';
    var self = this;
...
InitGame.prototype.update = function() {
    ...
    if (shieldHits < 1 && !self.gameOver) {
        ...
        self.game.world.find('/GameRoot/UI/Scoreboard/').getScript('qc.TweenPosition').playGroup();
        self.game.world.find('/GameRoot/UI/GameOver').getScript('qc.TweenAlpha').play();
        self.game.world.find('/GameRoot/UI/GameOver/GameOver').getScript('qc.TweenAlpha').play();
        self.game.world.find('/GameRoot/UI/GameOver/PlayAgain').getScript('qc.TweenAlpha').play();
    ...
    if (!self.gameOver) {
        t = this.bulletCount || self.game.world.find('/GameRoot/UI/Scoreboard/Time');
		t.text = ((this.game.time.now - this.startedAt) / 1000).toFixed(2);
    }
    
    var t = this.shieldHitText || self.game.world.find('/GameRoot/UI/ShieldHits');
    t.text = '' + shieldHits;
    
    v = self.game.storage.get('asteroidsDetroyed') || 0;
    t = this.asteroidsDetroyedText || self.game.world.find('/GameRoot/UI/Scoreboard/AsteroidsDestroyed');
    t.text = '' + v;
    
    v = self.game.storage.get('bulletCount') || 0;
    t = this.bulletCount || self.game.world.find('/GameRoot/UI/BulletCount');
    t.text = '' + v;   
